...
func TestTrieForkGC(t *testing.T) {
	...
	db := rawdb.NewMemoryDatabase()
	genesis := (&Genesis{BaseFee: big.NewInt(params.InitialBaseFee)}).MustCommit(db)
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, 2*DefaultTriesInMemory, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	blocks, _ := GenerateChain(&chainConfig, genesis, genEngine, db, 2*DefaultTriesInMemory, func(i int, gen *BlockGen) {
	...
	fork, _ := GenerateChain(&chainConfig, parent, genEngine, db, 2*DefaultTriesInMemory, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{2})
	})
...
func TestSideImportPrunedBlocks(t *testing.T) {
	...
	// Generate and import the canonical chain
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, 2*DefaultTriesInMemory, nil)
	diskdb := rawdb.NewMemoryDatabase()
...
func TestSetCanonical(t *testing.T) {
	...
	// Generate and import the canonical chain
	canon, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, 2*DefaultTriesInMemory, func(i int, gen *BlockGen) {
	...
	side, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, 2*DefaultTriesInMemory, func(i int, gen *BlockGen) {
