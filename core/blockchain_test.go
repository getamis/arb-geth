...
func testLargeReorgTrieGC(t *testing.T, scheme string) {
	...
	db, _ := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), t.TempDir(), "", false, false)
	defer db.Close()
...
func testLowDiffLongChain(t *testing.T, scheme string) {
	...
	diskdb, _ := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), t.TempDir(), "", false, false)
	defer diskdb.Close()
...
func TestTransactionIndices(t *testing.T) {
	...
	for _, l := range limit {
		...
		rawdb.WriteAncientBlocks(ancientDb, append([]*types.Block{gspec.ToBlock()}, blocks...), append([]types.Receipts{{}}, receipts...), big.NewInt(0), []*types.TransferLog{})
...
func TestTxIndexer(t *testing.T) {
	...
	for _, c := range cases {
		...
		rawdb.WriteAncientBlocks(db, append([]*types.Block{gspec.ToBlock()}, blocks...), append([]types.Receipts{{}}, receipts...), big.NewInt(0), []*types.TransferLog{})
