...
func (p *BlobPool) Reset(oldHead, newHead *types.Header) {
	...
	arbosVersion := types.DeserializeHeaderExtraInformation(newHead).ArbOSFormatVersion
	// Flush out any blobs from limbo that are older than the latest finality
	if p.chain.Config().IsCancun(newHead.Number, newHead.Time, arbosVersion) {
	...
	if !p.chain.Config().IsOsaka(oldHead.Number, oldHead.Time, types.DeserializeHeaderExtraInformation(oldHead).ArbOSFormatVersion) && p.chain.Config().IsOsaka(newHead.Number, newHead.Time, types.DeserializeHeaderExtraInformation(newHead).ArbOSFormatVersion) {
...
func (p *BlobPool) reinject(addr common.Address, txhash common.Hash) error {
	...
	head := p.head.Load()
	if p.chain.Config().IsOsaka(head.Number, head.Time, types.DeserializeHeaderExtraInformation(head).ArbOSFormatVersion) && tx.BlobTxSidecar().Version == types.BlobSidecarVersion0 {
		if err := tx.BlobTxSidecar().ToV1(); err != nil {
...
func (p *BlobPool) preCheck(tx *types.Transaction) error {
	var (
		head     = p.head.Load()
		isOsaka  = p.chain.Config().IsOsaka(head.Number, head.Time, types.DeserializeHeaderExtraInformation(head).ArbOSFormatVersion)
		deadline time.Time
	)
	if isOsaka {
