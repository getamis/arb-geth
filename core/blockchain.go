...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, statedb *state.StateDB) error {
	...
	root, stateUpdate, err := statedb.CommitWithUpdate(block.NumberU64(), bc.chainConfig.IsEIP158(block.Number()), bc.chainConfig.IsCancun(block.Number(), block.Time(), types.DeserializeHeaderExtraInformation(block.Header()).ArbOSFormatVersion))
	if err != nil {
		return err
	}
...
func (bc *BlockChain) logForkReadiness(block *types.Block) {
	...
	current := bc.Config().LatestFork(block.Time(), arbosVersion)
...
func (bc *BlockChain) InsertHeadersBeforeCutoff(headers []*types.Header) (int, error) {
	...
	if first == 1 && frozen == 0 {
		_, err := rawdb.WriteAncientBlocks(bc.db, []*types.Block{bc.genesisBlock}, []rlp.RawValue{rlp.EmptyList}, [][]*types.TransferLog{{}})
		if err != nil {
			log.Error("Error writing genesis to ancients", "err", err)
			return 0, err
		}
