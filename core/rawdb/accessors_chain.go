...
func ReadTransferLogs(db ethdb.KeyValueReader, hash common.Hash, number uint64) []*types.TransferLog {
	...
	data, _ := db.Get(append(append(blockTransferLogsPrefix, encodeBlockNumber(number)...), hash.Bytes()...))
	if len(data) == 0 {
		return nil
	}
...
func WriteTransferLogs(db ethdb.KeyValueWriter, hash common.Hash, number uint64, transferLogs []*types.TransferLog) {
	...
	key := append(append(blockTransferLogsPrefix, encodeBlockNumber(number)...), hash.Bytes()...)
...
func DeleteTransferLogs(db ethdb.KeyValueWriter, hash common.Hash, number uint64) {
	if err := db.Delete(append(append(blockTransferLogsPrefix, encodeBlockNumber(number)...), hash.Bytes()...)); err != nil {
