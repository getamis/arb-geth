...
func ReadTransferLogs(db ethdb.Reader, hash common.Hash, number uint64, returnNotFoundError bool) ([]*types.TransferLog, error) {
	// Retrieve the flattened transfer log slice
	transferLogs := []*types.TransferLog{}
	data := ReadTransferLogsRLP(db, hash, number)
	if len(data) == 0 {
		if returnNotFoundError {
...
func WriteAncientBlocks(db ethdb.AncientWriter, blocks []*types.Block, receipts []rlp.RawValue, transferLogs [][]*types.TransferLog) (int64, error) {
	return db.ModifyAncients(func(op ethdb.AncientWriteOp) error {
		for i, block := range blocks {
			header := block.Header()
			if err := writeAncientBlock(op, block, header, receipts[i], transferLogs[i]); err != nil {
				return err
			}
		}
		return nil
