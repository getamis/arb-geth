...
func InspectDatabase(db ethdb.Database, keyPrefix, keyStart []byte) error {
	...
	inspectRange := func(ctx context.Context, r byte) error {
		...
		for it.Next() {
			...
			case bytes.HasPrefix(key, CodePrefix) && len(key) == len(CodePrefix)+common.HashLength:
				codes.add(size)
			case bytes.HasPrefix(key, blockTransferLogsPrefix) && len(key) == (len(blockTransferLogsPrefix)+8+common.HashLength):
	...
	stats := [][]string{
		...
		{"Key-Value store", "Transfer logs", transferLog.sizeString(), transferLog.countString()},
